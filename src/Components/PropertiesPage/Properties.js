...
import { useFavorite } from "../PropertiesPage/FavoriteContext";
...
const Properties = () => {
  ...
  const handleFavorites = (item) => {
    ...
    isItemInFavorites ? removeFromFavorites(item) : addToFavorites(item);
  ...
  const removeFromFavorites = (item) => {
    ...
    // this.setFavorites({
    //   favorites: this.favorites.filter((e) => {
    //     return e.id !== item.id;
    //   }),
    // });
  ...
  const writeToLocalStorage = (items) => {
    let storageItems = JSON.parse(localStorage.getItem("favorites")) ?? [];

    let itemsArray = [];

    if (Array.isArray(storageItems)) {
      itemsArray = storageItems;
    } else {
      itemsArray.push(storageItems);
    }
      ...
      <div className="container">
        <div className="section-title mt-5 d-lg-flex align-items-center">
          <div className="text-container mt-3 pt-3 ms-4 ps-4">
            ...
            >
              Discover Your Dream Home with SHEVIN HOMES
            </h2>
            <p
              style={{
                textAlign: "justify",
            ...
            >
              {" "}
              Explore and find your dream home on Moving right discover our wide
              range of beautful homes all From the comfort of your own home.
            </p>
          </div>
          <img
            className="d-lg-block d-sm-none w-100 mt-3"
        ...
        <Container fluid>
          ...
          <Row className="justify-content-center mt-4">
            {filteredProperties.map((property) => (
              <Card
                key={property.id}
                className="properties-card col-md-2 col-sm-10 m-4 p-0"
              >
                <Card.Img
                  variant="top"
